...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isAdminConfigured()) {
      ...
      console.error('Admin login not configured. Missing environment variables:', missing);
      return NextResponse.json(
        { success: false, message: 'Admin login is not configured. Please check server configuration.' },
        { status: 503 }
      );
    }
