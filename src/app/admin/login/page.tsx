      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-md w-full space-y-8">
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600 dark:text-gray-400">{t('Dashboard.checkingConfiguration')}</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        {state.step === 'email' ? (
          <form className={`mt-8 space-y-6 ${!state.configured ? 'opacity-50 pointer-events-none' : ''}`} onSubmit={handleEmailSubmit}>
            <div>
          ...
          <form className={`mt-8 space-y-6 ${!state.configured ? 'opacity-50 pointer-events-none' : ''}`} onSubmit={handleOTPSubmit}>
            <div>
              ...
              <input
                ...
                placeholder="000000"
                value={state.otp}
                onChange={(e) => updateState({ otp: e.target.value.replace(/\D/g, '') })}
                disabled={state.loading || !state.configured}
                tabIndex={!state.configured ? -1 : 0}
              />
            </div>
            ...
            <div className="flex space-x-4">
              ...
              <button
                type="submit"
                disabled={state.loading || state.otp.length !== 6 || !state.configured}
                className="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-accent-600 hover:bg-accent-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent-500 disabled:opacity-50 disabled:cursor-not-allowed"
                tabIndex={!state.configured ? -1 : 0}
              >
        ...
        {state.step === 'otp' && (
          <div className="text-center">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {t('Login.sentTo')} <span className="font-medium">{state.email}</span>
            </p>
            <button
              type="button"
              onClick={handleBackToEmail}
