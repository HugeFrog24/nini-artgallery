...
export default function AdminDashboard() {
  ...
  const updateArtistProfile = (updates: Partial<ArtistProfileWithTranslations>) => {
  ...
  const updateArtistTranslation = (locale: string, field: 'name' | 'description', value: string) => {
    if (!state.artistProfile) return;
  ...
  const addTranslation = (locale: string) => {
    if (!state.artistProfile || state.artistProfile.translations[locale]) return;
      ...
      if (data.success) {
        updateState({
          ...
          artistProfile: {
            ...
            name: data.data.name,
            description: data.data.description,
            translations: data.data.translations || state.artistProfile.translations
          }
        });
            ...
            {state.artistProfile && (
              <form onSubmit={handleArtistSave} className="mt-6 space-y-8">
                ...
                <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 border border-blue-200 dark:border-blue-800">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h4 className="text-lg font-medium text-blue-900 dark:text-blue-100">
                      Primary Language ({state.artistProfile.defaultLanguage.toUpperCase()})
                    </h4>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <FormField
                      ...
                      htmlFor="artistName"
                      characterCount={{ current: state.artistProfile.name.length, max: 100 }}
                    >
                      <input
                        type="text"
                        id="artistName"
                    ...
                    <FormField
                      ...
                      characterCount={{ current: state.artistProfile.description.length, max: 1000 }}
                    >
                      <textarea
                        id="artistDescription"
                        rows={6}
                        value={state.artistProfile.description}
                        onChange={(e) => updateArtistProfile({ description: e.target.value })}
                        maxLength={1000}
                ...
                <div>
                  ...
                  {/* Translation Cards */}
                  <div className="space-y-4">
                    {Object.entries(state.artistProfile.translations).map(([locale, translation]) => {
                      const localeInfo = getLocaleConfig(locale);
                      return (
                        <div key={locale} className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                          <div className="flex items-center justify-between mb-4">
                            <h5 className="font-medium text-gray-900 dark:text-white">
                              {localeInfo?.name || locale} ({locale.toUpperCase()})
                            </h5>
                            <button
                              type="button"
                          ...
                          <div className="space-y-4">
                            ...
                            <FormField
                              ...
                              htmlFor={`name-${locale}`}
                              characterCount={{ current: translation.name.length, max: 100 }}
                            >
                              <input
                                type="text"
                                id={`name-${locale}`}
                                value={translation.name}
                                onChange={(e) => updateArtistTranslation(locale, 'name', e.target.value)}
                                maxLength={100}
                                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-accent-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 transition-colors"
                            ...
                            <FormField
                              ...
                              characterCount={{ current: translation.description.length, max: 1000 }}
                            >
                              <textarea
                                id={`description-${locale}`}
                                rows={4}
                                value={translation.description}
                                onChange={(e) => updateArtistTranslation(locale, 'description', e.target.value)}
                                maxLength={1000}
                                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-accent-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 transition-colors resize-none"
                    ...
                    {Object.keys(state.artistProfile.translations).length === 0 && (
                      <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                        <GlobeAltIcon className="h-12 w-12 mx-auto mb-4 opacity-50" />
                        <p>No translations added yet.</p>
                        <p className="text-sm">Use the dropdown above to add translations in other languages.</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {state.personalMessage && (
                <form onSubmit={handleSave} className="mt-6 space-y-6">
                  ...
                  <FormField
                    ...
                    htmlFor="recipient"
                    characterCount={{ current: state.personalMessage.recipient.length, max: 100 }}
                  >
                    <input
                      type="text"
                      id="recipient"
                  ...
                  <FormField
                    ...
                    htmlFor="message"
                    characterCount={{ current: state.personalMessage.message.length, max: 2000 }}
                  >
                    <textarea
                      id="message"
                      rows={6}
