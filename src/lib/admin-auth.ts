...
import { getEmailTranslations } from './admin-i18n';
...
// Send OTP email
export async function sendOTPEmail(email: string, otp: string, locale?: string): Promise<boolean> {
  try {
    const emailTranslations = await getEmailTranslations(locale);
    const siteName = SMTP_FROM_NAME || 'Art Gallery';
    
    const mailOptions = {
      from: `${siteName} <${SMTP_CONFIG.auth.user}>`,
      to: email,
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <p style="color: #666; font-size: 12px;">${emailTranslations.automatedMessage.replace('{siteName}', siteName)}</p>
        </div>
      `,
